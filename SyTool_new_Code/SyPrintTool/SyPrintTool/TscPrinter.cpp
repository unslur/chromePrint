...
//	
//	try{
//		int ret = TSC_Setup("85","45","3","15","0","5","0");
//
...
//
//	try{
//		int ret = TSC_Setup("100","80","3","15","0","2","0");
//
//		TSC_Sendcommand("SET CUTTER OFF");
//		TSC_Sendcommand("DIRECTION 1");
//		ret = TSC_Clearbuffer();
...
//	return rtn;
//}
int CTscPrinter::Print3(char* printerName,const string& printStr,LONG nPrintType)
{
	int rtn;
	bool bIsPrint = false;
	char buff[0x100]={0};
	...
	//AFX_MANAGE_STATE(AfxGetStaticModuleState());
    try{
		int ret = TSC_Setup("100","25","3","13","0","2","2");
		ret = TSC_Clearbuffer();
		if ( ret != 1 ){
			sprintf(buff,"PTK_ClearBuffer:%d",ret);
			throw(buff);
		...
		if (printStr1.length() > 0 && (nPrintType == 1 || nPrintType == 4)){
			bIsPrint |= printN3(printStr1,X,Y,/*id_name,*/TSC_Sendcommand,TSC_Windowsfont);
		}
		//打印中间
		if (printStr2.length() > 0 && (nPrintType == 2 || nPrintType == 4)){
			bIsPrint |= printN3(printStr2,X-=272,Y,/*id_name,*/TSC_Sendcommand,TSC_Windowsfont);
		}
		//打印右边
		if (printStr3.length() > 0 && (nPrintType == 3 || nPrintType == 4)){
			bIsPrint |= printN3(printStr3,X-=272,Y,/*id_name,*/TSC_Sendcommand,TSC_Windowsfont);
		}
		//==================================
		if (bIsPrint){
			ret = TSC_Printlabel("1", "1");	
